...
Mat drawCircles(Mat img, vector<Circle> bestCircles, const int limit)
{
    ...
    cvtColor(img, result, COLOR_GRAY2BGR); // CV_GRAY2BGR
    
    for (int i=0; i < limit; ++i) {
        circle(result, Point(bestCircles[i].xCenter, bestCircles[i].yCenter), bestCircles[i].radius, Scalar(0,255,255),4);
    }
    return result;
}
...
int main(int argc, const char * argv[]) {
    ...
    Mat imgInput = imread("/home/miro/hcrc/jpg/Recon_BladeS1_Hel-Z3_u763.46_float_1200x1200x9000050.jpg", IMREAD_GRAYSCALE); // CV_LOAD_IMAGE_GRAYSCALE
    ...
    Canny(imgGaussian, imgEdges, 100, 255); // TODO do alternatives
    // local maxima of the vesselness

    imshow("edge", imgEdges);
    imwrite("edges.jpg", imgEdges);
    ...
    imwrite("HoughTransform.jpg", resultImgHough);
    ...
    imwrite("RANSAC.jpg", resultImgRansac);
